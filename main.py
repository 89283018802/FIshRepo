...
from forms import LoginForm, PoolForm, GroupPoolForm, HydrochemistryForm, FishTypeForm, \
                  ...
                  FeedFilterForm, FishMovementFilterForm, FishInventoryFilterForm,\
                  FishInventoryAndBoningForm, FishInventoryGraphForm
from models import db, User, Pool, GroupPool, Hydrochemistry, FishType,\
                   FishInventory, FishBoning, FeedType, Feed, FishMovement, group_pool_pool, MovementReason
...
def new_pool():
    form = PoolForm()
    if form.validate_on_submit():
        pool = Pool(name=form.name.data, square=form.square.data, deep=form.deep.data)
        db.session.add(pool)
        db.session.commit()
...
def edit_fish_type(fish_type_id):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('fish'))
    return render_template('edit_fish_type.html', form=form, fish_type=fish_type)

@app.route('/inventory', methods=['GET', 'POST'])
@login_required
def inventory():
    filter_form = FishInventoryFilterForm()
    query = FishInventory.query
    query_boning = FishBoning.query

    if filter_form.validate_on_submit():
        start_date = int(filter_form.start_date.data.timestamp())
        end_date = int(filter_form.end_date.data.timestamp())
        query = query.filter(FishInventory.control_date >= start_date, FishInventory.control_date <= end_date)
        query_boning = FishBoning.query.join(FishInventory, (FishInventory.id == FishBoning.fish_inventory_id)).filter(FishInventory.control_date >= start_date, FishInventory.control_date <= end_date)

    fish_inventories = query.all()
    fish_bonings = query_boning.all()
    return render_template('inventory.html', fish_inventories=fish_inventories, fish_bonings=fish_bonings, title='Инвентаризация', filter_form=filter_form)


@app.route('/inventory_and_boning/new', methods=['GET', 'POST'])
@login_required
def new_inventory_and_boning():
    form = FishInventoryAndBoningForm()
    ...
    if form.validate_on_submit():
        ...
        fish_boning = FishBoning(
            fish_inventory_id=fish_inventory.id,
            fish_number=form.fish_number.data,
            fish_biomass=form.fish_biomass.data,
            fish_survival=form.fish_survival.data,
            fish_biometric=form.fish_biometric.data,
    ...
    return render_template('edit_inventory_and_boning.html', form=form)


@app.route('/inventory_and_boning/edit/<int:inventory_id>', methods=['GET', 'POST'])
@login_required
def edit_inventory_and_boning(inventory_id):
    fish_inventory = FishInventory.query.get_or_404(inventory_id)
    fish_boning = FishBoning.query.filter_by(fish_inventory_id=inventory_id).first_or_404()
    form = FishInventoryAndBoningForm(obj=fish_inventory)
    ...
    if request.method == 'GET':
        ...
        form.fish_number.data = fish_boning.fish_number
        form.fish_biomass.data = fish_boning.fish_biomass
        form.fish_survival.data = fish_boning.fish_survival
        form.fish_biometric.data = fish_boning.fish_biometric
        form.fish_comment.data = fish_boning.fish_comment
    ...
    if form.validate_on_submit():
        ...
        fish_boning.fish_survival = form.fish_survival.data
        fish_boning.fish_biometric = form.fish_biometric.data
    ...
    return render_template('edit_inventory_and_boning.html', form=form, fish_inventory=fish_inventory, fish_boning=fish_boning)

@app.route('/inventory_and_boning/delete/<int:inventory_id>', methods=['POST'])
@login_required
def delete_inventory_and_boning(inventory_id):
    try:
        fish_inventory = FishInventory.query.get_or_404(inventory_id)
        fish_boning = FishBoning.query.filter_by(fish_inventory_id=inventory_id).first_or_404()
...
def feeding():
    ...
    if filter_form.validate_on_submit():
        start_date = int(filter_form.start_date.data.timestamp())
        end_date = int(filter_form.end_date.data.timestamp())
        query = query.filter(Feed.feed_date >= start_date, Feed.feed_date <= end_date)

    feeds = query.all()
    return render_template('feeding.html', feed_types=feed_types, filter_form=filter_form, feeds=feeds)
...
@login_required
def movement():
    filter_form = FishMovementFilterForm()
    query = FishMovement.query

    if filter_form.validate_on_submit():
        if filter_form.start_date.data:
            start_date = int(filter_form.start_date.data.timestamp())
            query = query.filter(FishMovement.movement_date >= start_date)
        if filter_form.end_date.data:
            end_date = int(filter_form.end_date.data.timestamp())
            query = query.filter(FishMovement.movement_date <= end_date)
        if filter_form.movement_reason.data:
            query = query.filter(FishMovement.movement_reason == filter_form.movement_reason.data)

    movements = query.all()
    return render_template('movement.html', movements=movements, filter_form=filter_form)
    ...
    if form.validate_on_submit():
        if form.pool_id_from.data == form.pool_id_to.data:
            ...
            fish_movement = FishMovement(
                ...
                fish_biomass=form.fish_biomass.data,
                movement_reason=MovementReason[form.movement_reason.data],  # Использование Enum здесь
...
def edit_fish_movement(movement_id):
    ...
    if form.validate_on_submit():
        if form.pool_id_from.data == form.pool_id_to.data:
            ...
            fish_movement.movement_reason = MovementReason[form.movement_reason.data]
...
def plot_graph():
    form = HydrochemistryGraphForm()
    inventory_form = FishInventoryGraphForm()
    plot = None
    inventory_plot = None
    parameter_display_name = None
    inventory_parameter_display_name = None
    ...
    if inventory_form.validate_on_submit():
        parameter = inventory_form.parameter.data
        inventory_parameter_display_name = dict(inventory_form.parameter.choices).get(parameter)
        start_date = inventory_form.start_date.data
        end_date = inventory_form.end_date.data

        # Convert dates to Unix timestamp
        start_timestamp = int(datetime.combine(start_date, datetime.min.time()).timestamp())
        end_timestamp = int(datetime.combine(end_date, datetime.min.time()).timestamp())

        data = FishBoning.query.join(FishInventory).filter(
            FishInventory.control_date.between(start_timestamp, end_timestamp)
        ).all()

        if data:
            inventory_plot = create_fish_inventory_plot(data, parameter, inventory_parameter_display_name)

    return render_template('plot_graph.html', form=form, inventory_form=inventory_form, plot=plot, inventory_plot=inventory_plot, parameter_display_name=parameter_display_name, inventory_parameter_display_name=inventory_parameter_display_name)
...
def create_fish_inventory_plot(data, parameter, parameter_display_name):
    traces = []
    pool_ids = {record.fish_inventory.pool_id for record in data}
    pools = {pool.id: pool.name for pool in Pool.query.filter(Pool.id.in_(pool_ids)).all()}

    for pool_id in pools:
        pool_data = [record for record in data if record.fish_inventory.pool_id == pool_id]
        pool_data.sort(key=lambda x: x.fish_inventory.control_date)
        x = [datetime.fromtimestamp(record.fish_inventory.control_date) for record in pool_data]
        y = [getattr(record, parameter) for record in pool_data]
